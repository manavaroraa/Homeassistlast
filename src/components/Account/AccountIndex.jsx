    ...
    <>
      <Constainer>
        ...
        {!profile?.user_premium && (
          <ContainerMakeAction
            title={lang?.accountIndex?.premiumUser?.title}
            description={lang?.accountIndex?.premiumUser?.text}
            buttonText={lang?.accountIndex?.premiumUser?.button}
        ...
        {!profile.trial &&
          ...
          !profile?.work_premium && (
            <ContainerMakeAction
              title={lang?.accountIndex?.keepYourProfile?.title}
              description={lang?.accountIndex?.keepYourProfile?.text}
              buttonText={lang?.accountIndex?.keepYourProfile?.button}
              onClick={() => {
        ...
        {!profile?.user_info?.type_home_assistent && (
          <ContainerMakeAction
            title={lang?.accountIndex?.homeAssistant?.title}
            description={lang?.accountIndex?.homeAssistant?.text}
            buttonText={lang?.accountIndex?.homeAssistant?.button}
            onClick={() => {
              setBecomeUser(false);
        ...
        {!profile?.user_info?.type_work && (
          <ContainerMakeAction
            title={lang?.accountIndex?.RegisterProfile?.title}
            description={lang?.accountIndex?.RegisterProfile?.text}
            buttonText={lang?.accountIndex?.RegisterProfile?.button}
            onClick={() => {
              setBecomeUser(false);
        ...
        <Box>
          <Label>{lang?.accountIndex?.hideProfile?.title}</Label>
          <CustomRadio
            name="hiddenProfile"
        ...
        <CustomTable>
          <Text fontWeight={600} marginLeft={2} fontSize="1.2rem">
            {lang?.accountIndex?.lastPayments?.title}
          </Text>
          <TableHead
            labels={[
              { title: lang?.accountIndex?.lastPayments?.datePayment },
              { title: lang?.accountIndex?.lastPayments?.valuePayment },
              { title: lang?.accountIndex?.lastPayments?.typePayment },
              { title: lang?.accountIndex?.lastPayments?.statusPayment },
            ]}
          />
