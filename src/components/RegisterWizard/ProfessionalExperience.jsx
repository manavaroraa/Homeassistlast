    ...
    if (values) {
      ...
      for (const item of daysTrue) {
        if (item !== "working_days") {
          ...
          let breakVal =
            formik.values.user_available_times?.timeing_days?.split(" ");
    ...
    <MyForm onSubmit={formik.handleSubmit} autocomplete="off">
      {formik.values.type_work && (
        <Grid width={width}>
          ...
          <MyInput
            ...
            label={lang?.accountProfile?.professionalExperience?.time}
            placeholderTemplate={lang?.accountProfile?.placeholder}
            name="experience_time"
            value={formik.values.experience_time}
            onChange={formik.handleChange}
            onBlur={formik.handleBlur}
            infoMessage={lang?.tooltips?.xpPeriod}
          />
          <MyInput
            isInvalid={
            ...
            onChange={formik.handleChange}
            onBlur={formik.handleBlur}
            infoMessage={lang?.tooltips?.courses}
          />
          <MyInput
            isInvalid={
            ...
            onChange={formik.handleChange}
            onBlur={formik.handleBlur}
            infoMessage={lang?.tooltips?.education}
          />
          <Box>
            <LabelWithTooltip
              label={lang?.accountProfile?.professionalExperience?.cvFile}
              infoMessage={lang?.tooltips?.attach}
            />
      ...
      {formik.values.type_home_assistent && (
        <Grid width={width}>
          <MyInput
            ...
            label={lang?.accountProfile?.whyHA}
      ...
      <Grid width={width}>
        <Box>
          <LabelWithTooltip
            label={
              lang?.accountProfile?.professionalExperience?.availableTime?.label
            }
            infoMessage={lang?.tooltips?.hours}
          />
          <Grouping>
              ...
              <CustomSelect
                ...
                onChange={(e) => {
                  if (e.target.value.length > 0) {
                    for (const date of userAvailableTimes) {
                      for (const field of Object.values(date)) {
